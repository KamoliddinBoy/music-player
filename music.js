...
const and = document.getElementById('end')
...
function setProgress(e) {
    ...
    end.textContent = `${minutes}:${(seconds = seconds < 10 ? '0' + seconds : seconds)}`

    let currentMinutes = Math.floor(currentTimeMusic / 60)
    let currentSecond = Math.floor(currentTimeMusic % 60)
    start.textContent = `${currentMinutes}:${(currentSecond = currentSecond < 10 ? '0' + currentSecond : currentSecond)}`
}
